...
**All seventeen domains now carry canon** (28 files, 329 facts) — `celebrity` was the last empty one
...
What remains is depth, not coverage. One domain still sits **below the 12-fact retrieval cap** —
`peoples` 10 — and below that line a `domain_floor` seat pulls
